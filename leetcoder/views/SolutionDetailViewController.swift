...
class SolutionDetailViewController: BaseViewController, WKNavigationDelegate  {
    var slug:String = "liang-shu-zhi-he-by-leetcode-2"
    var markdown: String = ""
    ...
    func setupViews(){
        ...
        contentWebView.scrollView.backgroundColor = UIColor.clear
    ...
    func fetchData() {
        beginLoading()
        let parameters = [
            "query": GraphqlSchema.cnSolutionDetail,
        ...
        MySession.request(Urls.graphql.withCN(), method:.post, parameters: parameters, encoding: JSONEncoding.default, headers: getHeaders()).responseJSON { response in
            ...
            case let .success(value):
                self.data = JSON(value)["data"]["solutionArticle"]
                let code = self.data["content"].stringValue
    ...
    func loadWebView(_ code: String) {
        let down = Down(markdownString: code)
        let htmlContent = try? down.toHTML()
        var html = markdownHtml.replace(target: "--content--", withString: htmlContent ?? "")
        if #available(iOS 13.0, *) {
            let color = UIColor.init(dynamicProvider: { (trainCollection) -> UIColor in
                if trainCollection.userInterfaceStyle == .dark {
//                            main color
                    html = html.replace(target: "color: #24292e", withString: "color: #A7A7A7")
//                            border color
                    html = html.replace(target: "#e1e4e8", withString: Colors.base.toHexString())
//                            pre background color
                    html = html.replace(target: "#f6f8fa", withString: Colors.mainColor.toHexString())
                    return .clear
                } else {
                    return .clear
                }
